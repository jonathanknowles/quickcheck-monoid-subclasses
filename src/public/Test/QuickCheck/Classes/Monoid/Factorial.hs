...
factorialMonoidLaws _ = Laws "FactorialMonoid"
    ...
    , makeLaw1 @a
        ...
        (factorialMonoidLaw_splitAt)
    , makeLaw1 @a
        "factorialMonoidLaw_spanMaybe_unit"
        (factorialMonoidLaw_spanMaybe_unit)
    , makeLaw2 @a
        "factorialMonoidLaw_spanMaybe_foldl"
        (factorialMonoidLaw_spanMaybe_foldl)
    , makeLaw2 @a
        "factorialMonoidLaw_spanMaybe_prefix_suffix"
        (factorialMonoidLaw_spanMaybe_prefix_suffix)
    ]
...
factorialMonoidLaw_spanMaybe_unit
    ...
    => a
    -> Property
factorialMonoidLaw_spanMaybe_unit a = withShowableFn inner
  where
    inner p =
        makeProperty
            "lhs == rhs"
            (lhs == rhs)
        & report
            "lhs: (spanMaybe () (const $ bool Nothing (Just ()) . p) a)"
                  (spanMaybe () (const $ bool Nothing (Just ()) . p) a)
        & report
            "rhs: (takeWhile p a, dropWhile p a, ())"
                  (takeWhile p a, dropWhile p a, ())
        & report
            "takeWhile p a"
            (takeWhile p a)
        & report
...
factorialMonoidLaw_spanMaybe_foldl
    ...
    -> a
    -> Property
factorialMonoidLaw_spanMaybe_foldl s0 a0 =
    forAllBlind genAccumulatorFn $ \(fDefinition, f) ->
    makeProperty
        "spanMaybe s0 (λs a -> Just $ f s a) a0 == (a0, mempty, foldl f s0 a0)"
        (spanMaybe s0 (\s a -> Just $ f s a) a0 == (a0, mempty, foldl f s0 a0))
    & report
        "f"
        (fDefinition)
    & report
    ...
    & report
        ...
        (a0)
    & report
        "spanMaybe s0 (λs a -> Just $ f s a) a0"
        (spanMaybe s0 (\s a -> Just $ f s a) a0)
    & report
        "foldl f s0 a0"
        (foldl f s0 a0)
...
factorialMonoidLaw_spanMaybe_prefix_suffix
    ...
    -> a
    -> Property
factorialMonoidLaw_spanMaybe_prefix_suffix s0 a0 =
    forAllBlind genAccumulatorMaybeFn $ \(fDefinition, f) ->
    makeProperty
        "spanMaybe s0 f a0 & λ(p, s, _) -> a0 == p <> s"
        (spanMaybe s0 f a0 & \(p, s, _) -> a0 == p <> s)
    & report
        "f"
        (fDefinition)
    & report
        ...
        (spanMaybe s0 f a0)
    & report
        "spanMaybe s0 f a0 & λ(p, s, _) -> p <> s"
        (spanMaybe s0 f a0 & \(p, s, _) -> p <> s)
...
genAccumulatorMaybeFn :: Semigroup a => Gen (String, a -> a -> Maybe a)
genAccumulatorMaybeFn = elements
    [ ( "λ_ _ -> Nothing"
      , (\_ _ -> Nothing)
      )
    , ( "λa _ -> Just a"
